%function [pL_ele,tstart,tend,t] = noice_ele_corde_twist(c,Omega,twist,Rind,COOR_POLAR)
clear;
global dtau
global Nb_disc
...
params = minparametre;
c = params(1:Nb_disc);
Omega = params(2*Nb_disc+1);
twist = params(Nb_disc+1:2*Nb_disc);
B = params(2*Nb_disc+2);
Rind = 6;

rtip = 0.12;
%twist = [45 34 29 23 18 16 14 13 12]*pi/180;
%c = [0.17 0.22 0.23 0.22 0.2 0.17 0.15 0.1 0.07]*rtip;
%Omega = 5000;
c0 = 340;
U0 = 0;     % axial velocity

[dT,dD,r,dr] = noice_aerody(c,twist,Omega,rtip,Nb_disc);
Omega = 2*pi*Omega/60;
dT = dT(Rind);
dD = dD(Rind);

% dtau = 1e-3;    % discretization step for source time 
phi1 = 0;       % initial azimutal position
tau = 0:dtau:1;
R = r(Rind);
y1 = U0*tau;
y2 = R*cos(phi1+Omega*tau);
y3 = R*sin(phi1+Omega*tau);

% coordonnee de Glegg&Devenport Page 408
PHI = COOR_POLAR(1);
THETA = COOR_POLAR(2);
RAYON = COOR_POLAR(3);
x1 = RAYON*sin(THETA)*cos(PHI);      % x1:x
x3 = RAYON*sin(THETA)*sin(PHI);     % x3:y
x2 = RAYON*cos(THETA);           % x2:z
...
tend = max(t);
M = Omega*R/c0;
Mr = -x2/x*M*sin(Omega*tau);
dMr_dtau = -x2/x*M*Omega*cos(Omega*tau);

dDoverdT = dD/dT;
Cp = x1/x./(1-Mr).^3/Omega.*dMr_dtau +...
        -x2/x*dDoverdT./(1-Mr).^2.*(cos(Omega*tau) + 1/Omega*dMr_dtau.*sin(Omega*tau)./(1-Mr));
pL_ele = Cp.*(dT*Omega/(4*pi*x*c0))*dr(Rind);

    function [dT,dD,r,dr]=noice_aerody(c,twist,n,rtip,Nb_disc)
