function [T]=constraint_aerody_corde_twist(params)
...
n_sec = n/60; % rotations par seconde
...
twist_deg = 180/pi*twist; % twist angle in deg
...
% integrate over dr to obtain total loads
% T = trapz(dr(~isnan(dT)),dT(~isnan(dT)))
% T = trapz(r(3:end),dT(3:end),2)
T = sum(dr(~isnan(dT)).*dT(~isnan(dT))); % thrust (N)
Q = sum(dr(~isnan(dT)).*dQ(~isnan(dT))); % torque (N.m)
P = Q*omega; % power (W)

% thrust and torque coefficient (definition of Casalino et al., 2021)
C_T = T/(rho*omega^2*pi*rtip^4);
C_Q = Q/(rho*omega^2*pi*rtip^5);
